...
public:
        ...
        , bottom_panel_message("")
    {
        ...
        this->new_button.register_on_mouse_in([this](Button& self, MouseState const& mouse) {
            self.set_color(LIGHT_GRAY_COLOR);
        });
        this->new_button.register_on_mouse_out([this](Button& self, MouseState const& mouse) {
            self.set_color(GRAY_COLOR);
        });

        this->load_button = Button(this->sdl_renderer, {44, 5}, {22, 22}, GRAY_COLOR, "assets/map_editor/load.png");
        this->load_button.register_on_mouse_in([this](Button& self, MouseState const& mouse) {
            self.set_color(LIGHT_GRAY_COLOR);
        });
        this->load_button.register_on_mouse_out([this](Button& self, MouseState const& mouse) {
            self.set_color(GRAY_COLOR);
        });

        this->save_button = Button(this->sdl_renderer, {68, 5}, {22, 22}, GRAY_COLOR, "assets/map_editor/save.png");
        this->save_button.register_on_mouse_in([this](Button& self, MouseState const& mouse) {
            self.set_color(LIGHT_GRAY_COLOR);
        });
        this->save_button.register_on_mouse_out([this](Button& self, MouseState const& mouse) {
            self.set_color(GRAY_COLOR);
        });
        ...
        this->tm0.register_on_mouse_clicked([this](Button&, MouseState const&){
        ...
        this->tm1.register_on_mouse_clicked([this](Button&, MouseState const&){
        ...
        this->tmI.register_on_mouse_clicked([this](Button&, MouseState const&){
            this->selected_section = INTERACTABLES_SECTION;
        });
        ...
        this->fill_all_button.register_on_mouse_clicked([this](Button&, MouseState const&) {
            if (
                mouse.just_left_clicked &&
                this->selected_tile != -1 &&
                (this->selected_section == BACKGROUND_SECTION || this->selected_section == FOREGROUND_SECTION)
            ) {
        ...
        this->right_arrow_button.register_on_mouse_clicked([this](Button&, MouseState const&) {
            this->start_tile_id += 1;
        });
    }
...
private:
    ...
    void draw_bottom_panel()
    {
        ...
        auto message = this->map_filename + " [" + std::to_string(this->map.width) + "x" + std::to_string(this->map.height) + "]" \
            " . Mouse position: " + std::to_string(mouse_on_world.x) + ", " + std::to_string(mouse_on_world.y) + " . " + this->bottom_panel_message;
    ...
    void draw_left_panel()
    {
        ...
        for (int i = 0; i < MAX_ELEMS; ++i) {
            ...
            for (auto const& [di, dj] : std::array<std::tuple<int, int>, 4>{{{+1, 0}, {-1, 0}, {0, +1}, {0, -1}}}) {
                gout(this->sdl_renderer, this->monogram, {10 + x_offset + 2 + di, 50 + y_offset + 2 + dj}, std::to_string(tile_id), {255, 255, 255});
            }
            gout(this->sdl_renderer, this->monogram, {10 + x_offset + 2, 50 + y_offset + 2}, std::to_string(tile_id), DARK_PURPLE_COLOR);
